...
from src.config.Utility import Grid
from src.preset.A_STAR import A_STAR
from src.preset.BFS import BFS
from src.preset.DFS import DFS
from src.preset.GA import *
...
class GameController:
    ...
    def ate_Prey(self):
        if self.snake.ate_Prey():
            self.snake.add_body_ai()
            self.change_Prey_location()

    def change_Prey_location(self):
        while True:
            self.snake.create_Prey()
            position = self.snake.get_Prey()
            inside_body = False
    ...
    def ate_Prey_GA(self, snake):
        if snake.ate_Prey():
            snake.add_body_ai()
            self.change_Prey_location_GA(snake)

    def change_Prey_location_GA(self, snake):
        while True:
            snake.create_Prey()
            position = snake.get_Prey()
            inside_body = False
