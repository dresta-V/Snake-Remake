...
from src.gui.GameController import GameController
...
class GameGUI:
    def __init__(self):
        ...
        self.font_name = "src/gui/fonts/SquareAntiqua-Bold.ttf"
        self.speed = 110
        self.speed_up = 80
    ...
    def draw_elements(self):
        ...
        if self.curr_menu.state != 'GA' or self.controller.model_loaded:  # Path Ai or trained GA
            Prey = self.controller.get_Prey_pos()
    ...
    def draw_Prey(self, Prey):
        x = int(Prey.x * CELL_SIZE)
        y = int(Prey.y * CELL_SIZE)

        Prey_rect = pygame.Rect(x, y, CELL_SIZE, CELL_SIZE)
        pygame.draw.rect(self.display, Prey_COLOR, Prey_rect)
